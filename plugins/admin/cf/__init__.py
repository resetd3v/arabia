...
async def cfcheck():
    ...
    try:
        with open(f"{__file__.replace('__init__.py', '')}records.txt", "r") as file:
            data = file.read()
...
async def cfPrint():
    ...
    if not resp["success"]:
        ...
        # y tf does pyenv not work on my server | match resp["error"]  case "ipValid"
    ...
    # epic naming
    await utils.LOGGER.log(stack()[0][3], embed.description, "info")
